...
function MapVisual({
  ...
  return (
    <div style={{ position: 'relative', borderRadius: 14, overflow: 'hidden', background: 'var(--warm-gray)', border: '1px solid var(--border-light)', boxShadow: 'var(--shadow)', height: 540 }}>
      {/* Map background */}
      <svg
        width="100%"
        height="100%"
      ...
      >
        ...
        <rect width="600" height="540" fill="url(#mapBg)" />
        <rect x="0"   y="0"   width="155" height="130" fill="#dfe4eb" />
        <rect x="170" y="0"   width="120" height="95"  fill="#dfe4eb" />
        <rect x="310" y="0"   width="290" height="80"  fill="#dfe4eb" />
        <rect x="0"   y="155" width="100" height="120" fill="#dfe4eb" />
        <rect x="120" y="130" width="140" height="100" fill="#dfe4eb" />
        <rect x="280" y="100" width="110" height="90"  fill="#dfe4eb" />
        <rect x="410" y="95"  width="190" height="115" fill="#dfe4eb" />
        <rect x="0"   y="295" width="130" height="100" fill="#dfe4eb" />
        <rect x="150" y="250" width="120" height="130" fill="#dfe4eb" />
        <rect x="290" y="210" width="150" height="110" fill="#dfe4eb" />
        <rect x="460" y="230" width="140" height="130" fill="#dfe4eb" />
        <rect x="0"   y="415" width="180" height="125" fill="#dfe4eb" />
        <rect x="200" y="400" width="130" height="140" fill="#dfe4eb" />
        <rect x="350" y="360" width="100" height="180" fill="#dfe4eb" />
        <rect x="470" y="380" width="130" height="160" fill="#dfe4eb" />
        <rect x="310" y="220" width="135" height="110" rx="8" fill="#c5d4c8" />
        <rect x="130" y="300" width="110" height="90"  rx="6" fill="#cddad0" />
        ...
        <line x1="0" y1="136" x2="600" y2="136" stroke="rgba(5,14,36,0.03)" strokeWidth="1" strokeDasharray="12,10" />
        <line x1="165" y1="0" x2="165" y2="540" stroke="rgba(5,14,36,0.03)" strokeWidth="1" strokeDasharray="12,10" />
        <line x1="300" y1="0" x2="300" y2="540" stroke="rgba(5,14,36,0.03)" strokeWidth="1" strokeDasharray="12,10" />
      </svg>
          ...
          >
            ...
            {highlight && (
              <div style={{
                ...
                <div style={{ fontWeight: 600, fontSize: '0.78rem', color: 'var(--navy-heading)', marginBottom: 2 }}>
...
export default function LiveDealMap() {
  ...
  return (
    <div style={{ borderTop: 'none', padding: '96px 40px', background: 'var(--white)' }}>
      <div style={{ maxWidth: 1160, margin: '0 auto' }}>

        {/* Section header */}
        <div style={{ marginBottom: 40 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 12 }}>
            Marketplace
          </div>
          ...
          <p style={{ fontSize: '0.97rem', color: 'var(--body-text)', lineHeight: 1.65, maxWidth: 460 }}>
        ...
        <div className="ldm-layout" style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: 20, alignItems: 'start' }}>
          ...
          <div>
            ...
            <form
              ...
              style={{ display: 'flex', gap: 8, background: 'var(--white)', border: '1px solid var(--border-med)', borderRadius: 12, padding: 5, marginBottom: 10, boxShadow: '0 1px 4px rgba(5,14,36,0.04)' }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, padding: '0 10px' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--muted-text)" strokeWidth="2" strokeLinecap="round">
                ...
                <input
                  ...
                  style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'inherit', fontSize: '0.9rem', color: 'var(--navy-heading)' }}
                />
              </div>
              <button
                type="submit"
                style={{ background: 'var(--navy-heading)', color: 'white', border: 'none', borderRadius: 10, padding: '10px 20px', fontFamily: 'inherit', fontWeight: 600, fontSize: '0.85rem', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0, transition: 'opacity 0.15s' }}
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 18 }}>
              {cities.map(c => (
                <button
                  ...
                  style={{ padding: '5px 12px', borderRadius: 20, border: '1px solid var(--border-med)', background: search === c ? 'var(--navy-heading)' : 'var(--white)', color: search === c ? 'white' : 'var(--body-text)', fontSize: '0.75rem', fontWeight: 500, fontFamily: 'inherit', cursor: 'pointer', transition: 'all 0.15s' }}
                >
                  {c}
                </button>
              ))}
            ...
            {/* Count label */}
            {results.length > 0 && (
              <div style={{ fontSize: '0.72rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--muted-text)', marginBottom: 12 }}>
            ...
            <div className="ldm-list" style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 440, overflowY: 'auto', paddingRight: 2 }}>
              {results.length > 0 ? results.map((l, i) => {
                ...
                return (
                  ...
                  >
                    {/* Thumbnail */}
                    <div style={{ position: 'relative', width: 100, minHeight: 100, flexShrink: 0, overflow: 'hidden', background: 'var(--warm-gray)' }}>
                      <Image
                        src={l.img}
                        alt={l.addr}
                        fill
                    ...
                    <div style={{ flex: 1, padding: '10px 12px', minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 6 }}>
                        <div style={{ fontWeight: 600, fontSize: '0.82rem', color: 'var(--navy-heading)', lineHeight: 1.3 }}>
                          {l.addr.split(',')[0]}
                        </div>
                        <div style={{ fontWeight: 700, fontSize: '0.82rem', color: 'var(--navy-heading)', flexShrink: 0 }}>{l.ask}</div>
                      </div>
                      <div style={{ fontSize: '0.68rem', color: 'var(--muted-text)', marginTop: 2 }}>{l.specs} · {l.type}</div>
                      <div style={{ display: 'flex', gap: 12, marginTop: 6 }}>
                        <span style={{ fontSize: '0.68rem', color: 'var(--muted-text)' }}>ARV <span style={{ fontWeight: 600 }}>{l.arv}</span></span>
                        <span style={{ fontSize: '0.68rem', color: 'var(--green)', fontWeight: 700 }}>{l.fee}</span>
                      </div>
                      <div style={{ fontSize: '0.62rem', color: 'var(--muted-text)', marginTop: 5, display: 'flex', alignItems: 'center', gap: 3, borderTop: '1px solid var(--border-light)', paddingTop: 5 }}>
                        <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="var(--border-med)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--muted-text)', fontSize: '0.88rem' }}>
            ...
            {results.length > 0 && (
              <button
                style={{
                  ...
                  background: 'var(--white)', border: '1px solid var(--border-med)', borderRadius: 10,
                  padding: '11px 20px', fontFamily: 'inherit', fontWeight: 600, fontSize: '0.84rem',
                  color: 'var(--body-text)', cursor: 'pointer', transition: 'all 0.15s',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--border-med)'; e.currentTarget.style.color = 'var(--navy-heading)' }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border-med)'; e.currentTarget.style.color = 'var(--body-text)' }}
              >
                View more listings
      ...
      <style>{`
        ...
        .ldm-list::-webkit-scrollbar-thumb { background: var(--border-med); border-radius: 2px; }
