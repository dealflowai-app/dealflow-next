...
function statusStyle(s: string) {
  switch (s) {
    ...
    case 'Dormant': return 'text-gray-500 bg-gray-100'
...
function sourceIcon(s: string) {
  switch (s) {
    case 'AI Outreach': return <span title="AI Outreach"><Zap className="w-3 h-3 text-blue-500" /></span>
...
function tagColor(t: string) {
  const colors: Record<string, string> = {
    'Phoenix': 'bg-orange-50 text-orange-700 border-orange-200',
    'Dallas': 'bg-blue-50 text-blue-700 border-blue-200',
    'Atlanta': 'bg-violet-50 text-violet-700 border-violet-200',
    'Tampa': 'bg-cyan-50 text-cyan-700 border-cyan-200',
    'Flip Buyer': 'bg-emerald-50 text-emerald-700 border-emerald-200',
    'Rental Buyer': 'bg-indigo-50 text-indigo-700 border-indigo-200',
    'Fast Closer': 'bg-amber-50 text-amber-700 border-amber-200',
    'POF Verified': 'bg-green-50 text-green-700 border-green-200',
    'VIP': 'bg-rose-50 text-rose-700 border-rose-200',
    'New Lead': 'bg-sky-50 text-sky-700 border-sky-200',
    'Land Buyer': 'bg-lime-50 text-lime-700 border-lime-200',
    'Multi-Family': 'bg-purple-50 text-purple-700 border-purple-200',
  }
...
function FilterSelect({ label, options }: { label: string; options: string[] }) {
  return (
    <div className="relative">
      <select className="appearance-none bg-white border border-gray-200 rounded-lg pl-3 pr-7 py-2 text-[0.78rem] text-gray-600 outline-none focus:border-blue-300 transition-colors cursor-pointer">
    ...
    <div>
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5 mb-3">
          <span className="text-[0.8rem] text-blue-700 font-medium">{selected.size} selected</span>
          <div className="flex items-center gap-1.5 ml-auto">
            {[
              ...
              <button key={a.label} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[0.76rem] font-medium text-blue-600 hover:bg-blue-100 bg-transparent border-0 cursor-pointer transition-colors">
                <a.icon className="w-3.5 h-3.5" />
                {a.label}
              </button>
            ))}
          </div>
          <button onClick={() => setSelected(new Set())} className="text-blue-400 hover:text-blue-600 bg-transparent border-0 cursor-pointer">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
        ...
        <table className="w-full min-w-[1000px]">
          <thead>
            <tr className="border-b border-gray-100">
              <th className="w-10 px-3 py-3">
                <input type="checkbox" checked={allSelected} onChange={toggleAll} className="accent-blue-600 w-3.5 h-3.5 cursor-pointer" />
              </th>
              {[
                ...
                <th
                  ...
                  className={`px-3 py-3 text-[0.68rem] text-gray-400 uppercase tracking-wide font-medium whitespace-nowrap ${
          ...
          <tbody>
            {buyers.map((b, i) => (
              <tr
                key={b.id}
                className={`${i < buyers.length - 1 ? 'border-b border-gray-50' : ''} hover:bg-gray-50/70 transition-colors`}
              >
                <td className="px-3 py-3">
                  <input type="checkbox" checked={selected.has(b.id)} onChange={() => toggle(b.id)} className="accent-blue-600 w-3.5 h-3.5 cursor-pointer" />
                </td>
                <td className="px-3 py-3">
                  <button onClick={() => onOpenDetail(b)} className="flex items-center gap-2 bg-transparent border-0 cursor-pointer text-left p-0 group">
                    <div className={`w-7 h-7 rounded-full bg-gradient-to-br ${b.color} flex items-center justify-center flex-shrink-0`}>
                      <span className="text-[0.52rem] font-medium text-white">{b.initials}</span>
                    </div>
                    <span className="text-[0.82rem] font-medium text-gray-800 group-hover:text-blue-600 transition-colors">{b.name}</span>
                  </button>
                </td>
    ...
    <div className="flex gap-3 overflow-x-auto pb-4 crm-pipeline">
      {stages.map(stage => {
        ...
        const closedCards = stage === 'Closed'
          ? [
              { id: 100, name: 'Kevin Nguyen', initials: 'KN', color: 'from-cyan-500 to-blue-600', markets: ['Dallas'], buyBox: 'SFR, $120K-$250K, Flip', score: 'A', daysInStage: 0 },
              { id: 101, name: 'Jessica Rivera', initials: 'JR', color: 'from-violet-500 to-purple-600', markets: ['Atlanta'], buyBox: 'MF, $200K-$400K, Rental', score: 'A', daysInStage: 0 },
              { id: 102, name: 'Marcus Thompson', initials: 'MT', color: 'from-blue-500 to-blue-600', markets: ['Dallas'], buyBox: 'SFR, $100K-$200K, Flip', score: 'A', daysInStage: 0 },
            ]
          : []
              ...
              <div className="space-y-2">
                {allCards.map(c => (
                  <button
                    ...
                    className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2.5 text-left cursor-pointer hover:shadow-md hover:-translate-y-0.5 transition-all group"
                  >
                    <div className="flex items-center gap-2 mb-1.5">
                      <div className={`w-6 h-6 rounded-full bg-gradient-to-br ${c.color} flex items-center justify-center flex-shrink-0`}>
                        <span className="text-[0.46rem] font-medium text-white">{c.initials}</span>
                      </div>
                      <span className="text-[0.78rem] font-medium text-gray-800 truncate group-hover:text-blue-600 transition-colors">{c.name}</span>
                    </div>
        ...
        >
          ...
          {hoverBuyer === b.id && (
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-[#12141a] border border-gray-700 rounded-lg px-3 py-2.5 shadow-xl z-10 min-w-[180px]">
              <div className="flex items-center gap-2 mb-1.5">
                <div className={`w-6 h-6 rounded-full bg-gradient-to-br ${b.color} flex items-center justify-center flex-shrink-0`}>
                  <span className="text-[0.46rem] font-medium text-white">{b.initials}</span>
                </div>
              ...
              <button
                ...
                className="w-full text-[0.72rem] font-medium text-blue-400 hover:text-blue-300 bg-transparent border-0 cursor-pointer text-left transition-colors"
              >
                View Profile →
              </button>
              {/* Arrow */}
...
function BuyerDetail({ buyer, onClose }: { buyer: Buyer; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end">
      ...
      <div className="relative w-[520px] h-full bg-white shadow-2xl overflow-y-auto crm-detail-panel">
        {/* Header */}
        <div className="px-6 pt-6 pb-4 border-b border-gray-100">
          ...
          <div className="flex items-center gap-3 mb-3">
            <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${buyer.color} flex items-center justify-center flex-shrink-0`}>
              <span className="text-[0.7rem] font-medium text-white">{buyer.initials}</span>
            </div>
            <div>
              <h2 className="text-[1.1rem] font-medium text-gray-900" style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}>
                {buyer.name}
              </h2>
        ...
        <div className="px-6 py-5 space-y-5">
          {/* Contact Info */}
          <div>
            <div className="text-[0.7rem] text-gray-400 uppercase tracking-wide mb-2.5 font-medium">Contact Info</div>
          ...
          {/* Buy Box */}
          <div>
            <div className="text-[0.7rem] text-gray-400 uppercase tracking-wide mb-2.5 font-medium">Buy Box</div>
            <div className="bg-gray-50 rounded-lg px-4 py-3 space-y-2">
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-[0.82rem] font-medium text-gray-800" style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}>{buyer.priceRange}</span>
              </div>
              <div className="flex items-center justify-between">
          ...
          {/* Activity Timeline */}
          <div>
            <div className="text-[0.7rem] text-gray-400 uppercase tracking-wide mb-2.5 font-medium">Activity Timeline</div>
          ...
          {/* Tags */}
          <div>
            <div className="text-[0.7rem] text-gray-400 uppercase tracking-wide mb-2.5 font-medium">Tags</div>
            <div className="flex flex-wrap gap-1.5">
              ...
              <button className="text-[0.72rem] text-gray-400 hover:text-blue-500 border border-dashed border-gray-300 hover:border-blue-300 rounded-full px-2.5 py-1 bg-transparent cursor-pointer transition-colors flex items-center gap-1">
    ...
    <div className="p-8 max-w-[1400px]">
      ...
      <div className="flex items-start justify-between mb-5">
        ...
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-1.5 bg-white border border-gray-200 hover:bg-gray-50 text-gray-600 rounded-lg px-4 py-2 text-[0.82rem] font-medium cursor-pointer transition-colors">
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <div className="relative w-[260px]">
          ...
          <input
            ...
            className="w-full bg-white border border-gray-200 rounded-lg pl-10 pr-4 py-2 text-[0.82rem] text-gray-700 placeholder-gray-400 outline-none focus:border-blue-300 transition-colors"
          />
        </div>
